...
export default async function Page() {
  ...
  // User check is handled by middleware, so user should exist here
  if (!user) {
  ...
  // Fetch user goals
  let { data: goals } = await supabase.from("goals").select("*").eq("user_id", user.id).single()
  ...
  if (!goals) {
    const { data: newGoals } = await supabase.from("goals").insert({
      user_id: user.id,
    }).select().single()
    goals = newGoals
  }
